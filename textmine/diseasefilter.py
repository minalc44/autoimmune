...
def correlate(name, phenotype_infile, xd, wd, recorrelate, prev_file):
    abstract_files = glob.glob(xd+'*.xml.gz')
    correlated = glob.glob(wd+'pm_xml/*'+name+'pubmed_correlation_hits_regex.txt')

    prev_file_pmids = []
    with open(xd+prev_file, 'rU') as infile:
        for line in infile:
            temp = line.strip('\n').split('\t')
            print temp
            prev_file_pmids.append(temp[1])
...
def run_pub_finder(wd, xd, term_infile, name, date, recorrelate, prev_file):
    if not os.path.exists(wd + 'pm_xml/'):
        os.mkdir(wd + 'pm_xml/')
    correlate(name, term_infile, xd, wd, recorrelate, prev_file)
    hitsassemble(xd, name, date)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-i", "--previous_correlation_file", action="store", dest="prev_file")
    args = parser.parse_args()

    run_pub_finder(args.wd, args.xd, args.term_infile, args.name, args.date, args.recorrelate, args.prev_file)
